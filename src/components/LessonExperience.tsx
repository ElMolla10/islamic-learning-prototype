...
export function LessonExperience({ lesson }: { lesson: Lesson }) {
  ...
  const lessonStartTracked=useRef(false);const completionTracked=useRef(false);
  ...
  useEffect(()=>{const restored=parseProgress(localStorage.getItem(PROGRESS_KEY));const sessionFocus=sessionStorage.getItem(FOCUS_KEY)==="true";const initialCard=lesson.blocks.some(block=>block.key===restored.currentCardId)?restored.currentCardId:"card-1";
    completionTracked.current=restored.lessonCompleted;
    if(!restored.lessonOpened&&!lessonStartTracked.current){lessonStartTracked.current=true;const storedLanguage=localStorage.getItem("islamic-library-language");trackAnalytics("lesson_start",{lesson_slug:LESSON_ID,language:storedLanguage==="en"?"en":"ar"});}
  ...
  useEffect(()=>{if(!hydrated.current||!progress.lessonCompleted||completionTracked.current)return;completionTracked.current=true;trackAnalytics("lesson_complete",{lesson_slug:LESSON_ID,language});},[language,progress.lessonCompleted]);
  ...
  return <SourceProvider sources={lesson.sources} analyticsContext={{lessonSlug:LESSON_ID,cardIndex:currentIndex+1,cardCount:lesson.blocks.length}}><div className={progress.focusMode?"guided-lesson focus-mode":"guided-lesson"}>
