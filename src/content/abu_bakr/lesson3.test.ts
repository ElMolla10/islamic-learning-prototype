...
describe("Abu Bakr Lesson 3 v2 public package", () => {
  ...
  it("makes Lesson 3 available in the existing display-ordered path", () => {
    ...
    expect(abuBakrPath.slice(0, 6).map((chapter) => chapter.state)).toEqual(["active", "active", "active", "active", "active", "active"]);
    expect(abuBakrPath.slice(6).every((chapter) => chapter.state === "planned")).toBe(true);
